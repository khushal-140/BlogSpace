...
```bash
python seed.py          # seeds only when the database has no posts
python seed.py --fresh  # wipes users + posts and reseeds
```

A fresh database also seeds itself automatically on startup (set
`SEED_DEMO_DATA=0` to disable), so deployments start with a lively blog.
